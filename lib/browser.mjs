...
async function readOne(raw, cache) {
  ...
  const pendingList = () =>
    [...pending].slice(0, 20).map((r) => ({ type: r.resourceType(), url: r.url().slice(0, 200) }))
  const mergeRefused = () => {
  ...
  try {
    ...
    mergeRefused()
    return { url: raw, ...out, blocked, ms: Date.now() - t0, ...(out.error ? { pending: pendingList() } : {}) }
  } catch (e) {
    mergeRefused()
    ...
    return main
      ...
      : { blocked, error: "page-failed", ms: Date.now() - t0, pending: pendingList(), url: raw, why: firstLine(e) }
  } finally {
    clearTimeout(timer)
