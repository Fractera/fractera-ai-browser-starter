...
// 🔒 ЗАПРЕТ АДРЕСОВ — НА КАЖДОМ ЗАПРОСЕ КОНТЕКСТА (196-3): `route` видит документ, фреймы, скрипты, картинки, `fetch` и
// перенаправления; `routeWebSocket` — сокеты, которых `route` не видит; `serviceWorkers: "block"` — потому что запросы
// сервис-воркера мимо `route` проходят (оговорка самого Playwright). Отказ записывается в `blocked` ответа: зовущий видит,
// что страница хотела достать и не достала.
...
async function readOne(raw, cache) {
  ...
  let timer
  try {
    context = await b.newContext({ acceptDownloads: false, serviceWorkers: "block" })
    ...
    const main = blocked.items.find((x) => x.main)
    return main
