// ЧТЕНИЕ СТРАНИЦ НАСТОЯЩИМ БРАУЗЕРОМ (шаги 196-2, 196-3).
//
// 🎯 СЛОВО ВЛАДЕЛЬЦА 2026-09-13: «извлечь именно весь доступный исходный код (текстовую составляющую, все что касается
// интерактивных элементов или медиафайлов проводится поверхностный анализ через атрибуты или заголовки) а не только Мета
// записи»; «обрабатывать нужно уметь ни одну ссылку а несколько ссылок».
//
// 🔒 ПОДКЛЮЧЕНИЕ ДЕРЖИТСЯ, А НЕ ОТКРЫВАЕТСЯ НА КАЖДЫЙ ВЫЗОВ: замер 196-1 — первое открытие 8 с, следующие ≈ 2 с. Движок
// перезапустился — адрес сменился, и подключение пересоздаётся. Несколько вкладок разом подключаются ОДНИМ обещанием.
// 🔒 КАЖДАЯ ССЫЛКА — СВОЙ КОНТЕКСТ: куки и хранилище одной страницы не достаются следующей.
// 🔒 «ДОЖДАТЬСЯ ЗАГРУЗКИ» ДВУХСТУПЕНЧАТО: `load` обязателен, тишина сети — не дольше `IDLE_MS`. Страницы с вечными
// запросами (аналитика, чаты) иначе не дают ответа никогда.
// 🔒 ЗАПРЕТ АДРЕСОВ — ДВУМЯ СЛОЯМИ (196-3). Держит ПРОКСИ контекста (`lib/egress.mjs`): через него идёт весь выход браузера,
// и мимо него не пройти. `route` и `routeWebSocket` стоят перед ним ради типа запроса в ответе. ✗ Одного `route` не хватило —
// измерено: перенаправление на петлю он в Firefox не видит, и слушатель на петле получил обращение.
// `serviceWorkers: "block"` — запросы сервис-воркера мимо `route` проходят. Отказы обоих слоёв пишутся в `blocked`: зовущий
// видит, что страница хотела достать и не достала.
// 🔒 ЭЛЕМЕНТЫ И МЕДИА — ПО АТРИБУТАМ, А НЕ СОДЕРЖИМЫМ: картинка отдаётся адресом и `alt`, видео — адресом, постером и
// длительностью. Скачивать медиа служба не обязана (слово владельца: «поверхностный анализ»).
// 🔒 ПРЕДЕЛЫ НАЗВАНЫ В ОТВЕТЕ, А ОБРЕЗАННОЕ — ПОМЕЧЕНО: список из 300 ссылок при 2 000 на странице без `total` читался бы
// как «ссылок 300».

import { firefox } from "playwright-core"
import { endpoint, restartEngine } from "./engine.mjs"
import { randomUUID } from "node:crypto"
import { sharedEgress, startEgress } from "./egress.mjs"
import { checkUrl, hostRefusal } from "./guard.mjs"

export const LIMITS = Object.freeze({
  blocked: 50,
  htmlChars: 5_000_000,
  listItems: 300,
  perUrlMs: 90_000,
  // 🔒 ОДНА СТРАНИЦА ЗА РАЗ (решение владельца 2026-09-14: «однозначно запретить параллельную работу в браузере… у нас не
  // локальная разработка на ограниченном сервере»). Измерено: голый Camoufox с тремя вкладками разом зависает сам (196-8),
  // первоисточник — задача #553 «one context at a time». Число не поднимать без нового замера и слова владельца.
  tabs: 1,
  textChars: 1_000_000,
  urls: 10,
})
/**
 * Режимы, выбираемые измерением 196-8 (переменные окружения процесса службы). Видны в `/v1/health`.
 * `AI_BROWSER_EGRESS_MODE`: `per-link` (прокси на ссылку, прежний) · `shared` (один постоянный прокси).
 * `AI_BROWSER_ROUTE`: `0` — без перехвата `route`/`routeWebSocket` (запрет держит прокси).
 */
export const MODES = Object.freeze({
  egress: process.env.AI_BROWSER_EGRESS_MODE === "shared" ? "shared" : "per-link",
  route: process.env.AI_BROWSER_ROUTE !== "0",
})

const GOTO_TIMEOUT_MS = 45_000
const LOAD_MS = 25_000
const IDLE_MS = 10_000

let connected = null
let connectedTo = null
let connecting = null

async function browser() {
  const ws = endpoint()
  if (!ws) return null
  if (connected && connectedTo === ws && connected.isConnected()) return connected
  if (connecting && connectedTo === ws) return connecting
  connectedTo = ws
  connecting = firefox
    .connect(ws, { timeout: 30_000 })
    .then((b) => {
      connected = b
      return b
    })
    .finally(() => {
      connecting = null
    })
  return connecting
}

const firstLine = (e) => String(e?.message ?? e).split("\n")[0].slice(0, 200)

/** Хост записи отказа: полный адрес или `host:port` строки `CONNECT`. */
const hostOf = (x) => {
  try {
    return new URL(x.url).hostname.replace(/^\[|\]$/g, "")
  } catch {
    return String(x.url).replace(/:\d+$/, "").replace(/^\[|\]$/g, "")
  }
}

/**
 * Перехват: каждый запрос и сокет контекста проходит через запрет адресов.
 * 🔒 `routeLog` — ЖУРНАЛ САМОГО ПЕРЕХВАТА ДЛЯ ТРАССЫ ОТКАЗА (196-4): http-навигация зависала на `about:blank` с пустым
 * журналом прокси и нулём запросов, и назвать, застрял ли запрос в перехвате или раньше него, было нечем.
 */
async function guardContext(context, cache, blocked, routeLog = []) {
  const refuse = (type, url, why, main = false) => {
    blocked.total += 1
    if (blocked.items.length < LIMITS.blocked) blocked.items.push({ main, type, url: url.slice(0, 300), why })
  }
  await context.route("**/*", async (route) => {
    const req = route.request()
    const t0 = Date.now()
    const entry = { decision: "pending", ms: null, url: req.url().slice(0, 160) }
    if (routeLog.length < 40) routeLog.push(entry)
    const done = (decision) => {
      entry.decision = decision
      entry.ms = Date.now() - t0
    }
    let why = null
    try {
      const u = new URL(req.url())
      if (u.protocol === "data:" || u.protocol === "blob:") {
        done("continue-local")
        return await route.continue()
      }
      why = u.protocol === "http:" || u.protocol === "https:" ? await hostRefusal(u.hostname, cache) : `scheme ${u.protocol}`
    } catch {
      why = "not a URL"
    }
    if (!why) {
      done("continue")
      return route
        .continue()
        .then(() => {
          entry.sent_ms = Date.now() - t0
        })
        .catch((e) => {
          entry.error = String(e?.message ?? e).split("\n")[0].slice(0, 120)
        })
    }
    let main = false
    try {
      main = req.isNavigationRequest() && req.frame().parentFrame() === null
    } catch { /* запрос без кадра — не главный документ */ }
    refuse(req.resourceType(), req.url(), why, main)
    done("fulfill-403")
    // 🛑 ОТВЕТ 403, А НЕ `abort`: оборванный фрейм в Firefox способен не дать странице `load` вовсе.
    return route.fulfill({ body: `blocked: ${why}`, contentType: "text/plain", status: 403 }).catch(() => {})
  })
  await context.routeWebSocket(/.*/, async (ws) => {
    let why
    try {
      const u = new URL(ws.url())
      why = u.protocol === "ws:" || u.protocol === "wss:" ? await hostRefusal(u.hostname, cache) : `scheme ${u.protocol}`
    } catch {
      why = "not a URL"
    }
    if (why) {
      refuse("websocket", ws.url(), why)
      return ws.close({ code: 1008, reason: "blocked" })
    }
    ws.connectToServer()
  })
}

/** Исполняется ВНУТРИ страницы: заголовки, мета, интерактивные элементы и медиа по атрибутам. */
function EXTRACT(max) {
  const clip = (s, n = 500) => {
    if (s == null) return null
    const t = String(s).replace(/\s+/g, " ").trim()
    return t ? t.slice(0, n) : null
  }
  const abs = (v) => {
    if (!v) return null
    try {
      return new URL(v, document.baseURI).href
    } catch {
      return v
    }
  }
  const take = (sel, map) => {
    const all = Array.from(document.querySelectorAll(sel))
    return { items: all.slice(0, max).map(map), total: all.length }
  }
  const labelOf = (el) => {
    if (el.id) {
      const l = document.querySelector(`label[for="${CSS.escape(el.id)}"]`)
      if (l) return clip(l.innerText, 200)
    }
    const p = el.closest("label")
    return p ? clip(p.innerText, 200) : null
  }
  const sources = (m) =>
    Array.from(m.querySelectorAll("source")).map((s) => ({ src: abs(s.getAttribute("src")), type: s.getAttribute("type") }))
  const meta = {}
  for (const m of document.querySelectorAll("meta[name],meta[property],meta[http-equiv]")) {
    const k = m.getAttribute("name") || m.getAttribute("property") || m.getAttribute("http-equiv")
    if (k && !(k in meta)) meta[k] = clip(m.getAttribute("content"), 1000)
  }
  return {
    audios: take("audio", (a) => ({
      duration: Number.isFinite(a.duration) ? Math.round(a.duration) : null,
      sources: sources(a),
      src: a.currentSrc || abs(a.getAttribute("src")),
      title: clip(a.getAttribute("title") || a.getAttribute("aria-label"), 200),
    })),
    buttons: take('button,[role="button"],input[type="button"],input[type="submit"],input[type="reset"]', (b) => ({
      aria_label: b.getAttribute("aria-label"),
      disabled: b.disabled === true,
      name: b.getAttribute("name"),
      tag: b.tagName.toLowerCase(),
      text: clip(b.innerText || b.value, 200),
      type: b.getAttribute("type"),
    })),
    canonical: abs(document.querySelector('link[rel="canonical"]')?.getAttribute("href")),
    fields: take(
      'input:not([type="button"]):not([type="submit"]):not([type="reset"]):not([type="hidden"]),textarea,select',
      (el) => ({
        aria_label: el.getAttribute("aria-label"),
        form: el.form ? el.form.id || el.form.getAttribute("name") || "form" : null,
        id: el.id || null,
        label: labelOf(el),
        name: el.getAttribute("name"),
        options: el.tagName === "SELECT" ? Array.from(el.options).slice(0, 50).map((o) => clip(o.text, 100)) : undefined,
        placeholder: el.getAttribute("placeholder"),
        required: el.required === true,
        tag: el.tagName.toLowerCase(),
        type: el.getAttribute("type"),
      }),
    ),
    forms: take("form", (f) => ({
      action: abs(f.getAttribute("action")),
      fields: f.elements.length,
      id: f.id || null,
      method: (f.getAttribute("method") || "get").toLowerCase(),
      name: f.getAttribute("name"),
    })),
    headings: take("h1,h2,h3,h4,h5,h6", (h) => ({ level: Number(h.tagName[1]), text: clip(h.innerText, 300) })),
    iframes: take("iframe", (f) => ({ name: f.getAttribute("name"), src: abs(f.getAttribute("src")), title: clip(f.title, 200) })),
    images: take("img", (i) => ({
      alt: i.getAttribute("alt"),
      height: i.naturalHeight || null,
      loading: i.getAttribute("loading"),
      src: i.currentSrc || abs(i.getAttribute("src")),
      title: clip(i.getAttribute("title"), 200),
      width: i.naturalWidth || null,
    })),
    lang: document.documentElement.getAttribute("lang"),
    links: take("a[href]", (a) => ({
      href: abs(a.getAttribute("href")),
      rel: a.getAttribute("rel"),
      target: a.getAttribute("target"),
      text: clip(a.innerText, 200),
      title: clip(a.getAttribute("title"), 200),
    })),
    meta,
    videos: take("video", (v) => ({
      duration: Number.isFinite(v.duration) ? Math.round(v.duration) : null,
      poster: abs(v.getAttribute("poster")),
      sources: sources(v),
      src: v.currentSrc || abs(v.getAttribute("src")),
      title: clip(v.getAttribute("title") || v.getAttribute("aria-label"), 200),
    })),
  }
}

/**
 * ЗАЩИЩЁННАЯ ВКЛАДКА (196-3, выделена в 196-4): проверка адреса → свой прокси → контекст с перехватом → `run(page, url)` →
 * отображение отказов. Всё, что открывает страницы, открывает их ЗДЕСЬ — второй путь в браузер обошёл бы запрет адресов.
 * Отказ — полем `error` с причиной, а не исключением: соседние ссылки вызова не страдают.
 * @param {string} raw
 * @param {(page: import("playwright-core").Page, url: string) => Promise<object>} run — обязан вернуть `final_url`
 * @param {{ cookies?: object[] }} [options]
 */
export function withGuardedPage(raw, run, options = {}) {
  const enqueuedAt = Date.now()
  queueLength += 1
  const job = queueTail.then(async () => {
    const startedAt = Date.now()
    try {
      // 🔒 СТРАЖ ОЧЕРЕДИ (196-8): жёсткий предел на всё обращение, включая создание контекста и страницы. Без него движок,
      // замёрзший на `newContext`, остановил бы очередь навсегда — предел страницы стоит только на навигации.
      const out = await Promise.race([
        guardedPageNow(raw, run, options),
        new Promise((resolve) =>
          setTimeout(
            () => resolve({ error: "engine-hung", url: raw, why: `browser engine gave no answer in ${HARD_LIMIT_MS / 1000} s` }),
            HARD_LIMIT_MS,
          ),
        ),
      ])
      if (out.error === "engine-hung" || isHangSignature(out)) {
        restartEngine(out.error === "engine-hung" ? "no answer within the hard limit" : "navigation stuck on about:blank with no network")
      }
      return { ...out, queued_ms: startedAt - enqueuedAt }
    } finally {
      queueLength -= 1
    }
  })
  queueTail = job.catch(() => {})
  return job
}

// 🔒 ОДНА ОЧЕРЕДЬ НА ВЕСЬ ПРОЦЕСС СЛУЖБЫ (196-8, решение владельца 2026-09-14): страница за страницей — внутри вызова и между
// вызовами разных программ (`read`, `youtube`, стенд). Следующая начинается только после завершения предыдущей.
// Время ожидания очереди отдаётся в ответе (`queued_ms`) и в предел страницы не входит.
let queueTail = Promise.resolve()
let queueLength = 0

/** Жёсткий предел одного обращения: предел страницы плюс запас на контекст и уборку. */
const HARD_LIMIT_MS = LIMITS.perUrlMs + 20_000

/**
 * Признак зависшего движка (196-8, измерено): страница осталась `about:blank`, прокси ссылки не получил ни одного соединения и
 * браузер не выпустил ни одного запроса. Медленный или недоступный сайт этот признак не даёт — у него есть соединения прокси.
 */
function isHangSignature(out) {
  const t = out?.trace
  return Boolean(out?.error && t && t.page_url === "about:blank" && !(t.egress ?? []).length && !(t.requests ?? []).length)
}

/** Сколько обращений к браузеру сейчас в очереди, включая выполняемое. */
export function queueState() {
  return { length: queueLength }
}

async function guardedPageNow(raw, run, { cookies = [] } = {}) {
  const cache = undefined
  const t0 = Date.now()
  const blocked = { items: [], total: 0 }
  const checked = await checkUrl(raw, cache)
  if (!checked.ok) return { error: checked.error, ms: Date.now() - t0, url: raw, why: checked.why }

  // 🔒 ДВИЖОК ПЕРЕЗАПУСКАЕТСЯ — ЖДЁМ ЕГО ДО 30 С, А НЕ ОТКАЗЫВАЕМ СРАЗУ: после самолечения очередь не должна сыпаться отказами.
  for (let i = 0; i < 30 && !endpoint(); i++) await new Promise((resolve) => setTimeout(resolve, 1000))

  let b
  try {
    b = await browser()
  } catch (e) {
    return { error: "engine-connect-failed", ms: Date.now() - t0, url: raw, why: firstLine(e) }
  }
  if (!b) return { error: "engine-unreachable", ms: Date.now() - t0, url: raw }

  let context
  let timer
  let egress
  const navHosts = new Set()
  const requests = new Map()
  const routeLog = []
  let pageRef = null
  // 🔒 ТРАССА ОТКАЗА: адрес страницы в момент отказа, последние запросы с состоянием и причиной сбоя, журнал прокси.
  const trace = () => ({
    egress: egress?.log ?? [],
    page_url: pageRef?.url() ?? null,
    route: routeLog,
    requests: [...requests.entries()].slice(-30).map(([r, s]) => ({
      failure: s.failure ?? null,
      ms: s.ms ?? Date.now() - s.t,
      state: s.state,
      type: r.resourceType(),
      url: r.url().slice(0, 200),
    })),
  })
  const mergeRefused = () => {
    for (const x of egress?.refused.items ?? []) if (blocked.items.length < LIMITS.blocked) blocked.items.push(x)
    blocked.total += egress?.refused.total ?? 0
  }
  try {
    egress = MODES.egress === "shared" ? await sharedEgress(randomUUID()) : await startEgress()
    context = await b.newContext({
      acceptDownloads: false,
      proxy: {
        server: `http://127.0.0.1:${egress.port}`,
        ...(egress.username ? { password: egress.password, username: egress.username } : {}),
      },
      serviceWorkers: "block",
    })
    if (MODES.route) await guardContext(context, cache, blocked, routeLog)
    if (cookies.length) await context.addCookies(cookies)
    const page = await context.newPage()
    // Хосты цепочки главного документа, включая перенаправления: по ним отказ прокси отличают от отказа картинки.
    pageRef = page
    page.on("request", (r) => {
      requests.set(r, { state: "started", t: Date.now() })
      try {
        if (r.isNavigationRequest() && r.frame() === page.mainFrame()) navHosts.add(new URL(r.url()).hostname)
      } catch { /* запрос без кадра */ }
    })
    // 🔒 НЕЗАВЕРШЁННЫЕ ЗАПРОСЫ — ЧАСТЬ ОТКАЗА: «страница не загрузилась за 60 с» без списка того, что она ждала, не отвечает
    // на вопрос «почему». ✗ Первый прогон с прокси дал такой таймаут на витрине, и причину назвать было нечем.
    const settle = (state) => (r) => {
      const s = requests.get(r)
      if (!s) return
      s.state = state
      s.ms = Date.now() - s.t
      s.failure = r.failure()?.errorText
    }
    page.on("requestfinished", settle("finished"))
    page.on("requestfailed", settle("failed"))
    const work = Promise.resolve().then(() => run(page, checked.url))
    work.catch(() => {})
    const out = await Promise.race([
      work,
      new Promise((resolve) => {
        timer = setTimeout(() => resolve({ error: "page-timeout", why: `no answer in ${LIMITS.perUrlMs / 1000} s` }), LIMITS.perUrlMs)
      }),
    ])
    mergeRefused()
    // 🔒 ПЕРЕНАПРАВЛЕНИЕ НА ЗАПРЕЩЁННЫЙ АДРЕС ДАЁТ СТРАНИЦУ 403 ОТ ПРОКСИ, И ЭТО ОТКАЗ, А НЕ УСПЕХ (измерено третьим
    // прогоном: `final_url` на петле, код 403, `error` пуст). Зовущий получил бы наш текст отказа под видом страницы.
    if (!out.error && out.final_url) {
      const finalHost = hostOf({ url: out.final_url })
      const refusedMain = blocked.items.find((x) => hostOf(x) === finalHost)
      if (refusedMain) {
        return { blocked, error: "url-forbidden", final_url: out.final_url, ms: Date.now() - t0, url: raw, why: `redirect: ${refusedMain.why}` }
      }
    }
    return { url: raw, ...out, blocked, ms: Date.now() - t0, ...(out.error ? { trace: trace() } : {}) }
  } catch (e) {
    mergeRefused()
    // 🔒 ГЛАВНЫЙ ДОКУМЕНТ ОТВЕРГНУТ ЗАПРЕТОМ (перенаправление на петлю) — это `url-forbidden`, а не «страница упала»:
    // зовущий должен знать, что виноват адрес, а не браузер.
    const main = blocked.items.find((x) => x.main || navHosts.has(hostOf(x)))
    return main
      ? { blocked, error: "url-forbidden", ms: Date.now() - t0, url: raw, why: `redirect: ${main.why}` }
      : { blocked, error: "page-failed", ms: Date.now() - t0, trace: trace(), url: raw, why: firstLine(e) }
  } finally {
    clearTimeout(timer)
    // 🛑 ЗАКРЫТИЕ КОНТЕКСТА ЖДЁТСЯ С ПРЕДЕЛОМ: при мёртвом канале управления (задача #719) `close()` не вернётся никогда, и
    // при одной очереди на процесс это остановило бы службу целиком.
    await Promise.race([context?.close().catch(() => {}), new Promise((resolve) => setTimeout(resolve, 5000))])
    egress?.close()
  }
}

/** Страница целиком: итоговый HTML, текст, элементы и медиа по атрибутам. */
async function readBody(page, url) {
  // 🔒 ТРИ СТУПЕНИ ОЖИДАНИЯ, И ОТКАЗОМ БЫВАЕТ ТОЛЬКО ПЕРВАЯ (196-3, оплачено замером): `goto` ждёт документ
  // (`domcontentloaded`), затем `load` — не дольше `LOAD_MS`, затем тишина сети — не дольше `IDLE_MS`. ✗ Витрина через
  // прокси отдавала `load` за 47–65 с, и при трёх вкладках разом `goto` по `load` падал таймаутом — страница, текст
  // которой уже был в браузере, объявлялась неоткрытой. Не дождались `load` — отдаём что есть и говорим `load_reached`.
  const resp = await page.goto(url, { timeout: GOTO_TIMEOUT_MS, waitUntil: "domcontentloaded" })
  const loadReached = await page
    .waitForLoadState("load", { timeout: LOAD_MS })
    .then(() => true)
    .catch(() => false)
  await page.waitForLoadState("networkidle", { timeout: IDLE_MS }).catch(() => {})
  const html = await page.content()
  const text = await page.evaluate(() => (document.body ? document.body.innerText : ""))
  let parts = null
  let extractError = null
  try {
    parts = await page.evaluate(EXTRACT, LIMITS.listItems)
  } catch (e) {
    extractError = firstLine(e)
  }
  return {
    ...(parts ?? {}),
    ...(extractError ? { extract_error: extractError } : {}),
    final_url: page.url(),
    html: html.length > LIMITS.htmlChars ? html.slice(0, LIMITS.htmlChars) : html,
    html_length: html.length,
    html_truncated: html.length > LIMITS.htmlChars,
    load_reached: loadReached,
    status: resp ? resp.status() : null,
    text: text.length > LIMITS.textChars ? text.slice(0, LIMITS.textChars) : text,
    text_length: text.length,
    text_truncated: text.length > LIMITS.textChars,
    title: await page.title(),
  }
}

/** Ожидания защищённой вкладки — наружу для тех, кто открывает страницы своим `run` (YouTube). */
export const WAITS = Object.freeze({ gotoMs: GOTO_TIMEOUT_MS, idleMs: IDLE_MS, loadMs: LOAD_MS })

/**
 * Несколько ссылок: строго по очереди, порядок ответа = порядку ссылок (196-8, одна страница за раз).
 * Разрешение имён кэшируется в `guard.mjs` общим кэшем — второго кэша на вызов нет.
 */
export async function readPages(urls) {
  const results = []
  for (const url of urls) results.push(await withGuardedPage(url, readBody))
  return results
}
