...
// WebSocket разрешённой страницы иначе дошли бы до петли в обход. Перехват — в `lib/browser.mjs`.
// 🛑 ИЗВЕСТНЫЙ ПРЕДЕЛ, НАЗВАННЫЙ ВСЛУХ: имя разрешаем мы, соединяется браузер — между двумя разрешениями имя с нулевым
// TTL может сменить адрес (DNS rebinding). Закрыть это можно только сетевым уровнем машины (правило файрвола для
// процесса браузера); в службу это не входит.
