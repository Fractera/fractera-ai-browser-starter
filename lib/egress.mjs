...
/** Разрешить имя и выбрать адреса. Запрещён хоть один адрес имени — запрещено имя целиком. */
async function resolveAllowed(host) {
  const h = String(host).toLowerCase().replace(/^\[|\]$/g, "")
  if (!h) return { why: "empty-host" }
  if (isIP(h)) return isForbiddenAddress(h) ? { why: `forbidden-address ${h}` } : { addresses: [h] }
  let list
  try {
    // Сначала IPv4 — страховка, а не найденная причина: замер 196-3 показал, что IPv6 у сервера работает.
    list = await lookup(h, { all: true, order: "ipv4first" })
  } catch (e) {
  ...
  return { addresses: list.map((a) => a.address) }
}
