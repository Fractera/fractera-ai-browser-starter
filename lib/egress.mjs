...
async function resolveAllowed(host) {
  ...
  let list
  try {
    // 🔒 СНАЧАЛА IPv4: у сервера может не быть маршрута IPv6, и соединение с первым AAAA висит до таймаута страницы.
  ...
  return { address: list[0].address, addresses: list.map((a) => a.address) }
}
...
export async function startEgress() {
  ...
  server.on("connect", async (req, client, head) => {
    ...
    const up = await connectAny(r.addresses ?? [r.address], port, (s) => {
      track(s)
  ...
  server.on("upgrade", async (req, client, head) => {
    ...
    const up = await connectAny(r.addresses ?? [r.address], Number(u.port) || 80, (s) => {
      track(s)
