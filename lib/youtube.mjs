...
export async function readYouTube(url, lang) {
  ...
  const out = await withGuardedPage(
    ...
    async (page, target) => {
      ...
      if (!vd.videoId) {
        // 🔒 ПРОВЕРКА НА БОТОВ — НЕ «РОЛИКА НЕТ» (196-4, оплачено замером): существующий Big Buck Bunny вернул адресу
        // дата-центра `LOGIN_REQUIRED` «Sign in to confirm you’re not a bot», и первая редакция назвала это
        // `video-unavailable`. Зовущий решил бы, что ролик удалён, хотя он есть и откроется с другого адреса.
        const botCheck = playability.status === "LOGIN_REQUIRED" && /bot/i.test(playability.reason ?? "")
        return {
          ...base,
          error: botCheck ? "youtube-bot-check" : "video-unavailable",
          playability,
          why: playability.reason ?? `playability ${playability.status}`,
        }
      }
      const video = {
