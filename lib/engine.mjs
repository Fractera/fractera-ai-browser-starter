 ...
 *
 * 🔒 SIGKILL, А НЕ SIGTERM (196-8, оплачено прибором заморозки): замёрзший или остановленный процесс сигнал завершения не
 * обрабатывает — после самолечения на сервере осталось два родителя Camoufox.
 * 🔒 ПО ОБРАЗЦУ ИМЕНИ, А НЕ ТОЛЬКО ГРУППОЙ: Playwright запускает браузер в СВОЕЙ группе процессов, и `kill` группы `xvfb-run`
 * его не задевает — измерено: `frozen_pid_alive=yes` после убийства группы.
 */
function killOrphans() {
  for (const pattern of [`${PYTHON} -m camoufox server`, `${PYTHON} ${LAUNCH}`, `${CACHE}/camoufox`]) {
    try {
...
export function restartEngine(reason) {
  ...
  // Браузер живёт в своей группе — добиваем по образцу имени; новый движок поднимется позже, обработчиком выхода.
  killOrphans()
}
