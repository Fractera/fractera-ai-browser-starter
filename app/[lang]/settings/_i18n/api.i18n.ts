...
const EN: ApiDocWords = {
  ...
  refusals: {
    ...
    rows: [
      ...
      { code: "engine-hung", meaning: "per address: the browser engine stopped answering — the service restarts it by itself, the next address opens after it is up", status: "in results / 502" },
...
const RU: ApiDocWords = {
  ...
  refusals: {
    ...
    rows: [
      ...
      { code: "engine-hung", meaning: "по адресу: движок браузера перестал отвечать — служба сама перезапускает его, следующий адрес открывается после подъёма", status: "в results / 502" },
