...
const SELF_GUARDED = new Set([
  // 🪦 ДВЕРИ СТЕНДОВ, ОБЪЕКТОВ, ТЕРМИНАЛА И КЛЮЧЕЙ МОДЕЛЕЙ СЛУЖБЫ ПАМЯТИ СНЯТЫ (196-2) вместе с её логикой: служба
  // ИИ-браузера скопирована с памяти, и её двери имели бы здесь смысл только вместе с кодом, которого больше нет.
  // Закон этого списка остаётся: дверь, которая сама проверяет сессию, называется здесь до первой сборки.
  "/api/me",
]);
